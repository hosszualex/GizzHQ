...
class ValidatePasswordUseCaseTest {
    ...
    @Test
    fun `Password does not contain a number`() =
        runBlocking {
            Assert.assertFalse(validatePasswordUseCase("mybeautifulpasswordA"))
        }

    @Test
    fun `Password does not contain an uppercase letter`() =
        runBlocking {
            Assert.assertFalse(validatePasswordUseCase("mybeautifulpassword1"))
        }

    @Test
    fun `Password does not contain a lowercase letter`() =
        runBlocking {
            Assert.assertFalse(validatePasswordUseCase("MYBEAUTIFULPASSWORD1"))
        }
}
